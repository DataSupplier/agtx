...
- `DependencyState` (`db/models.rs`) — `Ready | Blocked(ids) | Missing(ids)`, computed by
  `Database::dependency_state`. An existing unmerged dependency outranks a deleted one,
...
- References double as **dependencies**: `Database::deps_satisfied` returns true only when every referenced task is **merged into its target** — `Task::satisfies_dependents`: Done, with no unresolved workflow integration (`Task::integration_status` unset — see *Workflow Integration* below). Review is not enough: a task under review is not merged, so a dependent started then would begin from a branch without its work. Starting research or moving a Backlog task forward is blocked until then (a warning is shown instead). `Database::dependency_state` answers the same question in more detail (`Ready` / `Blocked(ids)` / `Missing(ids)`) and is what the board's Ready lane projects from
...
- `build_resume_command()` is the recovery variant used after a tmux/server restart — mostly `--continue` appended to the launch flags (`ResumeArgs::Append`), but Gemini uses `--resume`. Codex and OpenCode resume **only by an explicit session id** found by `agent::native_session::session_id_for_worktree` (the session started in the task's own worktree): Codex `resume <id>` replaces the launch flags (`ResumeArgs::ReplaceWithSession`), OpenCode appends `--session <id>` (`ResumeArgs::AppendSession`). Without an id they start fresh — never `codex resume --last` (newest session in *any* directory) or `opencode --continue` (all worktrees share one OpenCode project), which resumed another task's conversation into a recovered pane. Recovery journals `agent_session_resumed` / `agent_resume_fresh_fallback` events
- **Workflow hand-offs are verified end to end** (`workflow_executor::hand_off_and_deliver`; every executor hand-off goes through it). A hand-off (1) waits while the pane's agent is mid-turn (`native_session::turn_activity`: Codex `task_started`/`task_complete`, OpenCode's `idle` message, Claude's hook status; up to `MAX_IDLE_DEFERRALS` automation ticks, journalled as `agent_handoff_deferred`), because agents write their artifact *before* their turn ends; (2) switches with `switch_agent_in_tmux(…, current, destination, cmd)`, which accepts only the **destination** agent's process as launched; (3) pastes a prompt only after the destination reports ready; and (4) confirms the destination's own session store recorded the prompt (`native_session::find_delivered_prompt`), else `agent_prompt_unconfirmed` and a retryable error before any evidence is persisted or state advanced. The confirmed session id is what `provider_sessions` records for that state attempt.
- `RealTmuxOps::pane_current_command` resolves the agent from the pane's **process tree** (`tmux::pane_process`), not tmux's `#{pane_current_command}`: agents under the `sh -c` launch wrapper report `sh`, and npm-installed agents (Codex) report `node`. Reading those as "at the shell" is how a busy Codex once received the next agent's launch line and prompt as a chat message.
- Recovery is **state-aware** (`resolve_recovery_session`): a rebuilt pane resumes the session that holds the current workflow state's stored prompt; if none does, the state's agent starts fresh and the prompt is delivered again (`agent_state_prompt_redelivered`). The web/TUI `resend_prompt` action re-delivers it manually, only into a pane running the task's own agent.
